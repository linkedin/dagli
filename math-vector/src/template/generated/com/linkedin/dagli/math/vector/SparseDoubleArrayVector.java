...
public class SparseDoubleArrayVector extends AbstractVector implements Serializable {
   ...
   */
  public SparseDoubleArrayVector(Vector other) {
   ...
   */
  private SparseDoubleArrayVector(VectorElementIterator iterator, int nonZeroCount) {
   ...
   */
  private SparseDoubleArrayVector(Tuple2<long[], double[]> indexAndValueArrays) {
    this(indexAndValueArrays.get0(), indexAndValueArrays.get1(), true);
  }

  private static Tuple2<long[], double[]> indexAndValueArraysFromVectorIterator(VectorElementIterator iterator,
      int nonZeroCount) {
    final long[] indices = new long[nonZeroCount];
    final double[] values = new double[nonZeroCount];
    int[] offset = new int[1];

    iterator.forEachRemaining((index, value) -> {
      indices[offset[0]] = index;
      values[offset[0]] = value;
      offset[0]++;
    });
  ...
  /**
   * Creates a {@link SparseDoubleArrayVector} from the provided arrays, which need not be sorted and can contain zero
   * values.  If duplicate indices are present and correspond to different non-zero values, which value is used is
   * determined arbitrarily.
   *
   * Unlike {@link #SparseDoubleArrayVector(long[], double[])}, here <strong>the vector takes ownership of the passed
   ...
   */
  public static SparseDoubleArrayVector wrap(long[] indices, double[] values) {
    return new SparseDoubleArrayVector(filterSortAndDeduplicateArrays(indices, values));
  }
   ...
   */
  public SparseDoubleArrayVector(long[] indices, double[] values) {
   ...
   *         method.
   */
  private static Tuple2<long[], double[]> filterSortAndDeduplicateArrays(long[] indices, double[] values) {
  ...
  /**
   * Creates a {@link SparseDoubleArrayVector} from the provided arrays where:
   * (1) "indices" is sorted
   ...
   */
  private SparseDoubleArrayVector(long[] indices, double[] values, boolean dummyVar) {
    _indices = indices;
    _values = values;
  }
