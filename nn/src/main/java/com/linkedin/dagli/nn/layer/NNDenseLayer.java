...
/**
 * A standard fully-connected perceptron layer.
 */
@VisitedBy("NNLayerVisitor")
public class NNDenseLayer extends AbstractUnaryVectorLayer<DenseVector, NNDenseLayer>
...
//   */
//  public NNDenseLayer withLayerNormalization(boolean useLayerNormalization) {
...
//  }
   ...
   */
  public NNDenseLayer withDropoutProbability(double probability) {
  ...
  DynamicLayerConfig getDynamicConfig(Map<NNLayer<?, ?>, DynamicLayerConfig> ancestorConfigs,
    ...
    if (width == -1) {
      width = ancestorConfigs.get(getInputLayer()).getOutputShape()[0];
    }
    return DynamicLayerConfig.Builder.setOutputShape(new long[] { width }).build();
  }
   ...
   */
  public NNDenseLayer withActivationFunction(ActivationFunction activation) {
   ...
   * has another {@link NNDenseLayer} as its parent, its width will (by default) match the parent layer's width.
   *
   ...
   */
  public NNDenseLayer withUnitCount(int width) {
   ...
   */
  public NNDenseLayer stack(int... unitCounts) {
    Arguments.check(unitCounts.length > 0, "At least one unit count for at least one new layer must be provided");
    NNDenseLayer previous = this.withUnitCount(unitCounts[0]).withInput(this.getInputLayer());
   ...
   * @return a copy of this instance with the specified width
   */
  public NNDenseLayer withUnitCount(Producer<? extends Number> widthProvider) {
