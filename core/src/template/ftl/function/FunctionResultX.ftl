...
<#if (arity > 1)>import com.linkedin.dagli.util.function.Function1;</#if>
import com.linkedin.dagli.util.function.Function${arity};
 ...
 */
 @ValueEquality
public class <@c.FunctionResult arity /> extends AbstractPreparedTransformer${arity}<<@c.InputGenericArguments arity />, R, <@c.FunctionResult arity />> {
  private static final long serialVersionUID = 1;

  private Function${arity}.Serializable<<@c.InputGenericArguments arity />, R> _originalFunction = null;
  @IgnoredByValueEquality
  private Function${arity}.Serializable<<@c.InputGenericArguments arity />, R> _function = null;
  private boolean _nullOnNullInput = false;

  /**
   * Creates a new instance with no function and missing input<@c.s arity />.  These must be set via the withFunction(...) and
   * withInput<@c.s arity />(...) methods, respectively.
   */
  public FunctionResult${arity}() {
    super(<#list 1..arity as index>MissingInput.get()<#sep>, </#list>);
  }
   ...
   *        withFunctionUnsafe(...).
   */
  public FunctionResult${arity}(Function${arity}.Serializable<<@c.InputGenericArguments arity />, R> func) {
    this();
   ...
   */
  private void setFunction(Function${arity}.Serializable<<@c.InputGenericArguments arity />, R> func) {
   ...
   */
  public <@c.FunctionResult arity /> withNullResultOnNullInput() {
    return clone(c -> {
  ...
  public <@c.FunctionResult arity /> withInput<@c.s arity />(<@c.InputProducerList arity />) {
   ...
   *        withFunctionUnsafe(...).
   */
  public <@c.FunctionResult arity /> withFunction(Function${arity}.Serializable<<@c.InputGenericArguments arity />, R> func) {
   ...
   */
  public <@c.FunctionResult arity /> withFunctionUnsafe(Function${arity}.Serializable<<@c.InputGenericArguments arity />, R> func) {
    return clone(c -> {
      try {
  ...
  @Override
  public R apply(<@c.InputSuffixedParameters "input" arity />) {
    return _function.apply(<@c.InputSuffixedList "input" arity />);
  }

  <#macro AndThenType>FunctionResult${arity}<<@c.InputGenericArguments arity />, Q></#macro>
  /**
   * Returns a new {@link FunctionResult${arity}} that composes this instance's function with another "mapper" function such
   ...
   * @return a new {@link FunctionResult${arity}} that composes this instance's function with another "mapper" function
   */
  @SuppressWarnings("unchecked")
  public <Q> <@AndThenType /> andThen(Function1.Serializable<? super R, ? extends Q> mapper) {
    return (<@AndThenType />) clone(
        c -> ((<@AndThenType />) c).setFunction(_originalFunction.andThen(mapper.safelySerializable())));
  }
}
