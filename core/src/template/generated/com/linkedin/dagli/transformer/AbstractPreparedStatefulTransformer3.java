...
/**
 * Base class for prepared transformers of arity 3 that extends {@link AbstractPreparedTransformer3} to
 ...
 */
public abstract class AbstractPreparedStatefulTransformer3<A, B, C, R, Q, S extends AbstractPreparedTransformer3<A, B, C, R, S>>
    extends AbstractPreparedTransformer3<A, B, C, R, S> {

  private static final long serialVersionUID = 1;

  public AbstractPreparedStatefulTransformer3() {
    super();
  }

  public AbstractPreparedStatefulTransformer3(Producer<? extends A> input1, Producer<? extends B> input2,
  ...
  public R apply(A value1, B value2, C value3) {
    return apply(createExecutionCache(1), value1, value2, value3);
  }
   ...
   */
  protected abstract R apply(Q executionCache, A value1, B value2, C value3);
  ...
  protected void applyAll(Q executionCache, List<? extends A> values1, List<? extends B> values2,
    ...
    for (int i = 0; i < values1.size(); i++) {
      results.add(apply(executionCache, values1.get(i), values2.get(i), values3.get(i)));
    }
  }
  ...
  protected class InternalAPI extends AbstractPreparedTransformer3<A, B, C, R, S>.InternalAPI {
    @Override
    public R apply(Object executionCache, A value1, B value2, C value3) {
      return AbstractPreparedStatefulTransformer3.this.apply((Q) executionCache, value1, value2, value3);
    }

    @Override
    public Object createExecutionCache(long exampleCountGuess) {
      return AbstractPreparedStatefulTransformer3.this.createExecutionCache(exampleCountGuess);
    }
    ...
    public void applyAllUnsafe(Object executionCache, int count, Object[][] values, Object[] results) {
      AbstractPreparedStatefulTransformer3.this.applyAll((Q) executionCache, new UnmodifiableArrayList<>(
          (A[]) values[0], count), new UnmodifiableArrayList<>((B[]) values[1], count), new UnmodifiableArrayList<>(
          (C[]) values[2], count), new FixedCapacityArrayList<>(results));
    }
    ...
    public void applyAllUnsafe(Object executionCache, int count, List<? extends List<?>> values, List<? super R> results) {
      AbstractPreparedStatefulTransformer3.this.applyAll((Q) executionCache,
          (List<A>) Lists.limit(values.get(0), count), (List<B>) Lists.limit(values.get(1), count),
          (List<C>) Lists.limit(values.get(2), count), results);
    }

    @Override
    public int getPreferredMinibatchSize() {
      return AbstractPreparedStatefulTransformer3.this.getPreferredMinibatchSize();
    }
  }
}
