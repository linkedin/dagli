...
/**
 * Base class for prepared transformers of arity 1 that extends {@link AbstractPreparedTransformer1} to
 ...
 */
public abstract class AbstractPreparedStatefulTransformer1<A, R, Q, S extends AbstractPreparedTransformer1<A, R, S>>
  ...
  public AbstractPreparedStatefulTransformer1(Producer<? extends A> input1) {
    super(input1);
  }

  @Override
  public R apply(A value1) {
    return apply(createExecutionCache(1), value1);
  }
   ...
   */
  protected abstract R apply(Q executionCache, A value1);
   ...
   */
  protected void applyAll(Q executionCache, List<? extends A> values1, List<? super R> results) {
    ArrayList<R> result = new ArrayList<>(values1.size());
    for (int i = 0; i < values1.size(); i++) {
      results.add(apply(executionCache, values1.get(i)));
    }
  }
  ...
  protected class InternalAPI extends AbstractPreparedTransformer1<A, R, S>.InternalAPI {
    @Override
    public R apply(Object executionCache, A value1) {
      return AbstractPreparedStatefulTransformer1.this.apply((Q) executionCache, value1);
    }

    @Override
    public Object createExecutionCache(long exampleCountGuess) {
      return AbstractPreparedStatefulTransformer1.this.createExecutionCache(exampleCountGuess);
    }
    ...
    public void applyAllUnsafe(Object executionCache, int count, Object[][] values, Object[] results) {
      AbstractPreparedStatefulTransformer1.this.applyAll((Q) executionCache, new UnmodifiableArrayList<>(
          (A[]) values[0], count), new FixedCapacityArrayList<>(results));
    }
    ...
    public void applyAllUnsafe(Object executionCache, int count, List<? extends List<?>> values, List<? super R> results) {
      AbstractPreparedStatefulTransformer1.this.applyAll((Q) executionCache,
          (List<A>) Lists.limit(values.get(0), count), results);
    }
