 ...
 *
 * DAG1x1 is a <i>preparable</i> DAG.
 *
 ...
 *
 * DAG1x1 is also a {@link com.linkedin.dagli.transformer.PreparableTransformer} and can be used as
 ...
 */
@ValueEquality
public class DAG1x1<A, RA> extends AbstractPreparableTransformer1<A, RA, DAG1x1.Prepared<A, RA>, DAG1x1<A, RA>>
    implements PreparableDAGTransformer<RA, DAG1x1.Prepared<A, RA>, DAG1x1<A, RA>> {
   ...
   */
  DAG1x1(DAGStructure<RA> dag, Producer<? extends A> input1) {
    super(input1);
    _dag = dag;
  ...
  public class InternalAPI extends
      AbstractPreparableTransformer1<A, RA, DAG1x1.Prepared<A, RA>, DAG1x1<A, RA>>.InternalAPI implements
      PreparableDAGTransformer.InternalAPI<RA, DAG1x1.Prepared<A, RA>, DAG1x1<A, RA>> {
    @Override
   ...
   */
  public DAG1x1<A, RA> withExecutor(DAGExecutor executor) {
   ...
   */
  public DAG1x1<A, RA> withInput(Producer<? extends A> input1) {
   ...
   * @param <RA> the type of the first result
   */
  private static class Preparer<A, RA> extends AbstractBatchPreparer1<A, RA, DAG1x1.Prepared<A, RA>> {
    ...
    public PreparerResultMixed<PreparedTransformer1<A, RA>, DAG1x1.Prepared<A, RA>> finish(ObjectReader<A> inputs) {
  ...
  @Override
  protected Preparer1<A, RA, DAG1x1.Prepared<A, RA>> getPreparer(PreparerContext context) {
   ...
   *         preparation data
   */
  public Result<A, RA> prepareAndApply(Iterable<? extends A> values1) {
    DAGExecutionResult<RA, DAG1x1.Prepared<A, RA>> res =
        _executor.internalAPI().prepareAndApplyUnsafe(this, new ObjectReader[] { ObjectReader.wrap(values1) });
    return new Result<>((DAG1x1.Prepared<A, RA>) res.getPreparerResult().getPreparedTransformerForNewData(),
        res.getOutputs());
  }
   ...
   */
  public DAG1x1.Prepared<A, RA> prepare(Iterable<? extends A> values1) {
    return (DAG1x1.Prepared<A, RA>) _executor.internalAPI()
  ...
  * @param <RA> the type of the first result
  ...
  public static final class Result<A, RA> extends AbstractDAGResult1<RA> {
     ...
     */
    Result(DAG1x1.Prepared<A, RA> preparedDAG, ObjectReader<?>[] results) {
      super(results);
  ...
  * @param <RA> the type of the first result
   */
  @ValueEquality
  public static class Prepared<A, RA> extends AbstractPreparedTransformer1<A, RA, DAG1x1.Prepared<A, RA>> implements
      PreparedDAGTransformer<RA, DAG1x1.Prepared<A, RA>> {
    ...
    * @param input1 the {@link Producer} providing the first input to the transformer
     */
    Prepared(DAGStructure<RA> dag, Producer<? extends A> input1) {
      super(input1);
      _dag = dag;
      _executor = new LocalDAGExecutor();
    }
     ...
     */
    @Override
    public DAG1x1.Prepared<A, RA> withReduction(Reducer.Level level) {
      if (Reducer.Level.compare(level, _reductionLevel) >= 0) {
        return this;
      }
    ...
    public class InternalAPI extends AbstractPreparedTransformer1<A, RA, DAG1x1.Prepared<A, RA>>.InternalAPI implements
        PreparedDAGTransformer.InternalAPI<RA, DAG1x1.Prepared<A, RA>> {
      @Override
      public DAGStructure<RA> getDAGStructure() {
        return _dag;
      }
      ...
      @Override
      public DAG1x1.Prepared<A, RA> getInstance() {
        return DAG1x1.Prepared.this;
      }
    }

    @Override
    protected Collection<? extends Reducer<? super DAG1x1.Prepared<A, RA>>> getGraphReducers() {
      return Collections.singletonList(DAGTransformerReducer.INSTANCE);
    }
     ...
     * @return a copy of this instance that will use the provided executor
     */
    public DAG1x1.Prepared<A, RA> withExecutor(PreparedDAGExecutor executor) {
      return clone(r -> r._executor = executor);
    }
     ...
     * @return a copy of this instance that will have the specified parent
     */
    public DAG1x1.Prepared<A, RA> withInput(Producer<? extends A> input1) {
      return super.withAllInputs(input1);
    }
     ...
     */
    public Result<RA> applyAll(Iterable<? extends A> values1) {
      return new Result<RA>(_executor.internalAPI()
          .applyUnsafe(this, new ObjectReader[] { ObjectReader.wrap(values1) }));
    }

    @Override
    public RA apply(A value1) {
      try (
