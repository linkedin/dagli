 ...
 *
 * DAG1x3 is a <i>preparable</i> DAG.
 *
 ...
 *
 * DAG1x3 is also a {@link com.linkedin.dagli.transformer.PreparableTransformer} and can be used as
 ...
 */
@ValueEquality
public class DAG1x3<A, RA, RB, RC> extends
    AbstractPreparableTransformer1<A, Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>, DAG1x3<A, RA, RB, RC>>
    implements PreparableDAGTransformer<Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>, DAG1x3<A, RA, RB, RC>> {
   ...
   */
  DAG1x3(DAGStructure<Tuple3<RA, RB, RC>> dag, Producer<? extends A> input1) {
    super(input1);
    _dag = dag;
   ...
   */
  @Override
  public DAG1x3<A, RA, RB, RC> withReduction(Reducer.Level level) {
  ...
  public class InternalAPI
      extends
      AbstractPreparableTransformer1<A, Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>, DAG1x3<A, RA, RB, RC>>.InternalAPI
      implements
      PreparableDAGTransformer.InternalAPI<Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>, DAG1x3<A, RA, RB, RC>> {
    @Override
    ...
    @Override
    public DAG1x3<A, RA, RB, RC> getInstance() {
      return DAG1x3.this;
    }
  }

  @Override
  protected Collection<? extends Reducer<? super DAG1x3<A, RA, RB, RC>>> getGraphReducers() {
   ...
   */
  public DAG1x3<A, RA, RB, RC> withExecutor(DAGExecutor executor) {
   ...
   */
  public DAG1x3<A, RA, RB, RC> withInput(Producer<? extends A> input1) {
   ...
   * @param <RC> the type of the third result
   */
  private static class Preparer<A, RA, RB, RC> extends
      AbstractBatchPreparer1<A, Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>> {
    // the DAG being prepared
    private final DAG1x3<A, RA, RB, RC> _dag;
     ...
     */
    Preparer(DAG1x3<A, RA, RB, RC> dag) {
      _dag = dag;
    }
    ...
    public PreparerResultMixed<PreparedTransformer1<A, Tuple3<RA, RB, RC>>, DAG1x3.Prepared<A, RA, RB, RC>> finish(
  ...
  @Override
  protected Preparer1<A, Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>> getPreparer(PreparerContext context) {
   ...
   *         preparation data
   */
  public Result<A, RA, RB, RC> prepareAndApply(Iterable<? extends A> values1) {
    DAGExecutionResult<Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>> res =
        _executor.internalAPI().prepareAndApplyUnsafe(this, new ObjectReader[] { ObjectReader.wrap(values1) });
    return new Result<>((DAG1x3.Prepared<A, RA, RB, RC>) res.getPreparerResult().getPreparedTransformerForNewData(),
        res.getOutputs());
  }
   ...
   */
  public DAG1x3.Prepared<A, RA, RB, RC> prepare(Iterable<? extends A> values1) {
    return (DAG1x3.Prepared<A, RA, RB, RC>) _executor.internalAPI()
  ...
  * @param <RC> the type of the third result
   */
  public static final class Result<A, RA, RB, RC> extends AbstractDAGResult3<RA, RB, RC> {
    // the prepared DAG
    private final DAG1x3.Prepared<A, RA, RB, RC> _preparedDAG;

    /**
     * @return the {@link Prepared} DAG that was prepared (trained) on the provided preparation (training) data.
     */
    public DAG1x3.Prepared<A, RA, RB, RC> getPreparedDAG() {
      return _preparedDAG;
    }
     ...
     */
    Result(DAG1x3.Prepared<A, RA, RB, RC> preparedDAG, ObjectReader<?>[] results) {
      super(results);
  ...
  * @param <RC> the type of the third result
   */
  @ValueEquality
  public static class Prepared<A, RA, RB, RC> extends
      AbstractPreparedTransformer1<A, Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>> implements
      PreparedDAGTransformer<Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>> {
    ...
    * @param input1 the {@link Producer} providing the first input to the transformer
     */
    Prepared(DAGStructure<Tuple3<RA, RB, RC>> dag, Producer<? extends A> input1) {
      super(input1);
      _dag = dag;
      _executor = new LocalDAGExecutor();
    }
     ...
     */
    @Override
    public DAG1x3.Prepared<A, RA, RB, RC> withReduction(Reducer.Level level) {
      if (Reducer.Level.compare(level, _reductionLevel) >= 0) {
        return this;
      }
    ...
    public class InternalAPI extends
        AbstractPreparedTransformer1<A, Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>>.InternalAPI implements
        PreparedDAGTransformer.InternalAPI<Tuple3<RA, RB, RC>, DAG1x3.Prepared<A, RA, RB, RC>> {
      @Override
      public DAGStructure<Tuple3<RA, RB, RC>> getDAGStructure() {
        return _dag;
      }
      ...
      @Override
      public DAG1x3.Prepared<A, RA, RB, RC> getInstance() {
        return DAG1x3.Prepared.this;
      }
    }

    @Override
    protected Collection<? extends Reducer<? super DAG1x3.Prepared<A, RA, RB, RC>>> getGraphReducers() {
      return Collections.singletonList(DAGTransformerReducer.INSTANCE);
    }
     ...
     * @return a copy of this instance that will use the provided executor
     */
    public DAG1x3.Prepared<A, RA, RB, RC> withExecutor(PreparedDAGExecutor executor) {
      return clone(r -> r._executor = executor);
    }
     ...
     * @return a copy of this instance that will have the specified parent
     */
    public DAG1x3.Prepared<A, RA, RB, RC> withInput(Producer<? extends A> input1) {
      return super.withAllInputs(input1);
    }
