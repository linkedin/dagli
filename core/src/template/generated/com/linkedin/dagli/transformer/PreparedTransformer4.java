...
public interface PreparedTransformer4<A, B, C, D, R> extends Transformer4<A, B, C, D, R>, PreparedTransformer<R> {
  ...
  default ObjectReader<R> applyAll(Iterable<? extends A> values1, Iterable<? extends B> values2,
    ...
    ObjectReader
        .concatenate(Object[]::new, ObjectReader.wrap(values1), ObjectReader.wrap(values2), ObjectReader.wrap(values3),
            ObjectReader.wrap(values4)).lazyMap(Arrays::asList).forEachBatch(minibatchSize, batch -> {
  ...
  @Override
  PreparedTransformer4InternalAPI<A, B, C, D, R, ? extends PreparedTransformer4<A, B, C, D, R>> internalAPI();
  ...
  static <A, B, C, D, R> PreparedTransformer4<A, B, C, D, R> cast(
      PreparedTransformer4<? super A, ? super B, ? super C, ? super D, ? extends R> prepared) {
    // safe due to semantics of prepared transformers:
    return (PreparedTransformer4<A, B, C, D, R>) prepared;
  }
   ...
   */
  static <A, B, C, D, R> DAG4x1.Prepared<A, B, C, D, R> toDAG(PreparedTransformer4<A, B, C, D, R> transformer) {
    if (transformer instanceof DAG4x1.Prepared) {
      return (DAG4x1.Prepared<A, B, C, D, R>) transformer;
    }
