// AUTOGENERATED CODE.  DO NOT MODIFY DIRECTLY!  Instead, please modify the math/vector/SparseXArrayVector.ftl file.
 ...
 */
public class SparseFloatArrayVector extends AbstractVector implements Serializable {
   ...
   */
  public SparseFloatArrayVector(Vector other) {
   ...
   */
  private SparseFloatArrayVector(VectorElementIterator iterator, int nonZeroCount) {
   ...
   */
  private SparseFloatArrayVector(Tuple2<long[], float[]> indexAndValueArrays) {
    this(indexAndValueArrays.get0(), indexAndValueArrays.get1(), true);
  }

  private static Tuple2<long[], float[]> indexAndValueArraysFromVectorIterator(VectorElementIterator iterator,
      int nonZeroCount) {
    final long[] indices = new long[nonZeroCount];
    final float[] values = new float[nonZeroCount];
    int[] offset = new int[1];

    iterator.forEachRemaining((index, value) -> {
      indices[offset[0]] = index;
      values[offset[0]] = (float) value;
      offset[0]++;
    });
  ...
  /**
   * Creates a {@link SparseFloatArrayVector} from the provided arrays, which need not be sorted and can contain zero
   * values.  If duplicate indices are present and correspond to different non-zero values, which value is used is
   * determined arbitrarily.
   *
   * Unlike {@link #SparseFloatArrayVector(long[], float[])}, here <strong>the vector takes ownership of the passed
   ...
   */
  public static SparseFloatArrayVector wrap(long[] indices, float[] values) {
    return new SparseFloatArrayVector(filterSortAndDeduplicateArrays(indices, values));
  }
   ...
   */
  public SparseFloatArrayVector(long[] indices, float[] values) {
   ...
   *         method.
   */
  private static Tuple2<long[], float[]> filterSortAndDeduplicateArrays(long[] indices, float[] values) {
  ...
  /**
   * Creates a {@link SparseFloatArrayVector} from the provided arrays where:
   * (1) "indices" is sorted
   ...
   */
  private SparseFloatArrayVector(long[] indices, float[] values, boolean dummyVar) {
    _indices = indices;
    _values = values;
  }
   ...
   */
  private SparseFloatArrayVector() {
