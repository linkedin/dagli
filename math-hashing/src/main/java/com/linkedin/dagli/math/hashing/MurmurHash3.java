...
/**
 *  Murmurhash3 is a popular family of fast, non-cryptographic hashing functions.
 *
 *  This class was adapted from the public domain implementation of MurmurHash3 was taken from:
 *  https://github.com/yonik/java_util/blob/master/src/util/hash/MurmurHash3.java
 *
 *  This variant of MurmurHash goes to some pains to ensure consistency with the reference implementation.
 *  Non-compliant MurmurHash(-ish) implementations will likely be faster.
 *
 *  fmix32 and fmix64 are simple (but decent) 32- and 64-bit integer hashing routines in their own right.
 *
 *  The original Javadoc follows below.
 *
 *  <p>
...
public abstract class MurmurHash3 {
  ...
  /**
   * Hashes a 32-bit integer to a 32-bit integer hash.
   *
  ...
  /**
   * Hashes a 64-bit long to a 64-bit long hash.
   *
   * @param k the long value to hash
   * @return the hash
   */
  ...
  private static long getLongLittleEndian(byte[] buf, int offset) {
    return ((long) buf[offset + 7] << 56)   // no mask needed
        | ((buf[offset + 6] & 0xffL) << 48) | ((buf[offset + 5] & 0xffL) << 40) | ((buf[offset + 4] & 0xffL) << 32) | (
        (buf[offset + 3] & 0xffL) << 24) | ((buf[offset + 2] & 0xffL) << 16) | ((buf[offset + 1] & 0xffL) << 8) | ((
        buf[offset] & 0xffL));        // no shift needed
  }

  /**
   * Calculates the MurmurHash3_x86_32 hash.
   *
   * @param data the data to hash
   * @param offset where hashing begins in data
  ...
  /**
   * Calculates the MurmurHash3_x86_32 hash of the UTF-8 bytes of the String without actually encoding
   * the string to a temporary buffer.  This is more than 2x faster than hashing the result
   * of String.getBytes().
   * @param data the data to hash
   * @param offset the offset in data where hashing begins
   * @param len the number of bytes of data to hash
  ...
  /**
   * Calculates the MurmurHash3_x64_128 hash, placing the result in "out".
   *
