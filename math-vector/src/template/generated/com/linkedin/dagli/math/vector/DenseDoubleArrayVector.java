...
public final class DenseDoubleArrayVector extends AbstractVector implements MutableDenseVector, Cloneable {
  ...
  /**
   * Creates a {@link DenseDoubleArrayVector} that will have a capacity for the provided number of elements.
   ...
   */
  public DenseDoubleArrayVector(int capacity) {
    this(new double[capacity], true);
  }

  /**
   * Creates a {@link DenseDoubleArrayVector} that wraps the provided array of feature values.  The indices of these values in the
   * vector will match those in the array.
   *
   * <strong>Changes to the array will be reflected in this {@link DenseDoubleArrayVector} (and vice-versa).</strong>
   *
   * @param values the array of feature values.  {@link DenseDoubleArrayVector} will store a reference, not a copy, of this array.
   */
  public static DenseDoubleArrayVector wrap(double... values) {
    return new DenseDoubleArrayVector(values, true);
  }

  @Override
  public DenseDoubleArrayVector clone() {
    return new DenseDoubleArrayVector(_values); // constructor will make copy of the array
  }
  ...
  public boolean equals(Object other) {
    ...
    if (other instanceof DenseDoubleArrayVector) {
      double[] otherValues = ((DenseDoubleArrayVector) other)._values;
  ...
  /**
   * Creates a {@link DenseDoubleArrayVector} with the values of the provided Vector.  An exception will occur if any element index
  ...
  public DenseDoubleArrayVector(Vector vector) {
    this(vector instanceof DenseDoubleArrayVector ? ((DenseDoubleArrayVector) vector).getArray().clone() : vector
        .toDoubleArray(), true);
  }

  /**
   * Creates a {@link DenseDoubleArrayVector} from the provided array of feature values.  The indices of these values in the vector
   * will match those in the array.
   *
   * {@link DenseDoubleArrayVector} will store a <strong>copy</strong> of the passed array.  If you can tolerate linking the array
   * values to the vector values, {@link #wrap(double...)} is more efficient as it stores a reference.
   *
   * @param values the array of feature values.
   */
  public DenseDoubleArrayVector(double... values) {
   ...
   *
   * @param values the array wrapped by this {@link DenseDoubleArrayVector}
   * @param dummyVar unused parameter to distinguish this constructor from the public constructor above
   */
  private DenseDoubleArrayVector(double[] values, boolean dummyVar) {
    _values = Objects.requireNonNull(values, "values must not be null");
  }

  /**
   * Gets the underlying array of element values.  Unlike {@link #toDoubleArray()}, this method does not create
   * a new array.
   *
   * <strong>Changes to the returned array will be reflected in this {@link DenseDoubleArrayVector} (and vice-versa).</strong>
   */
  ...
  public void put(long index, double value) {
    _values[Math.toIntExact(index)] = value;
  }
  ...
  public void transformInPlace(VectorElementTransformer transformer) {
    for (int i = 0; i < _values.length; i++) {
      if (_values[i] != 0) {
        _values[i] = (double) transformer.transform(i, _values[i]);
      }
    }
  }
  ...
  public void addInPlace(Vector other) {
    if (other instanceof DenseDoubleArrayVector) { // can be optimized
      addInPlace((DenseDoubleArrayVector) other);
    } else { // do the standard thing
      MutableDenseVector.super.addInPlace(other);
    }
  }

  public void addInPlace(DenseDoubleArrayVector dense) {
    for (int i = 0; i < _values.length; i++) {
  ...
  public double dotProduct(Vector other) {
    if (other instanceof DenseDoubleArrayVector) {
      return dotProduct((DenseDoubleArrayVector) other);
    }
   ...
   */
  public double dotProduct(DenseDoubleArrayVector other) {
