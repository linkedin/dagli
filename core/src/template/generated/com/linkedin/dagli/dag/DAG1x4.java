 ...
 *
 * DAG1x4 is a <i>preparable</i> DAG.
 *
 ...
 *
 * DAG1x4 is also a {@link com.linkedin.dagli.transformer.PreparableTransformer} and can be used as
 ...
 */
@ValueEquality
public class DAG1x4<A, RA, RB, RC, RD>
    extends
    AbstractPreparableTransformer1<A, Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>, DAG1x4<A, RA, RB, RC, RD>>
    implements
    PreparableDAGTransformer<Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>, DAG1x4<A, RA, RB, RC, RD>> {
   ...
   */
  DAG1x4(DAGStructure<Tuple4<RA, RB, RC, RD>> dag, Producer<? extends A> input1) {
    super(input1);
    _dag = dag;
   ...
   */
  @Override
  public DAG1x4<A, RA, RB, RC, RD> withReduction(Reducer.Level level) {
  ...
  public class InternalAPI
      extends
      AbstractPreparableTransformer1<A, Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>, DAG1x4<A, RA, RB, RC, RD>>.InternalAPI
      implements
      PreparableDAGTransformer.InternalAPI<Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>, DAG1x4<A, RA, RB, RC, RD>> {
    @Override
    ...
    @Override
    public DAG1x4<A, RA, RB, RC, RD> getInstance() {
      return DAG1x4.this;
    }
  }

  @Override
  protected Collection<? extends Reducer<? super DAG1x4<A, RA, RB, RC, RD>>> getGraphReducers() {
   ...
   */
  public DAG1x4<A, RA, RB, RC, RD> withExecutor(DAGExecutor executor) {
   ...
   */
  public DAG1x4<A, RA, RB, RC, RD> withInput(Producer<? extends A> input1) {
   ...
   * @param <RD> the type of the fourth result
   */
  private static class Preparer<A, RA, RB, RC, RD> extends
      AbstractBatchPreparer1<A, Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>> {
    // the DAG being prepared
    private final DAG1x4<A, RA, RB, RC, RD> _dag;
     ...
     */
    Preparer(DAG1x4<A, RA, RB, RC, RD> dag) {
      _dag = dag;
    }
    ...
    public PreparerResultMixed<PreparedTransformer1<A, Tuple4<RA, RB, RC, RD>>, DAG1x4.Prepared<A, RA, RB, RC, RD>> finish(
  ...
  @Override
  protected Preparer1<A, Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>> getPreparer(PreparerContext context) {
   ...
   *         preparation data
   */
  public Result<A, RA, RB, RC, RD> prepareAndApply(Iterable<? extends A> values1) {
    DAGExecutionResult<Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>> res =
        _executor.internalAPI().prepareAndApplyUnsafe(this, new ObjectReader[] { ObjectReader.wrap(values1) });
    return new Result<>(
        (DAG1x4.Prepared<A, RA, RB, RC, RD>) res.getPreparerResult().getPreparedTransformerForNewData(),
        res.getOutputs());
  }
   ...
   */
  public DAG1x4.Prepared<A, RA, RB, RC, RD> prepare(Iterable<? extends A> values1) {
    return (DAG1x4.Prepared<A, RA, RB, RC, RD>) _executor.internalAPI()
  ...
  * @param <RD> the type of the fourth result
   */
  public static final class Result<A, RA, RB, RC, RD> extends AbstractDAGResult4<RA, RB, RC, RD> {
    // the prepared DAG
    private final DAG1x4.Prepared<A, RA, RB, RC, RD> _preparedDAG;

    /**
     * @return the {@link Prepared} DAG that was prepared (trained) on the provided preparation (training) data.
     */
    public DAG1x4.Prepared<A, RA, RB, RC, RD> getPreparedDAG() {
      return _preparedDAG;
    }
     ...
     */
    Result(DAG1x4.Prepared<A, RA, RB, RC, RD> preparedDAG, ObjectReader<?>[] results) {
      super(results);
  ...
  * @param <RD> the type of the fourth result
   */
  @ValueEquality
  public static class Prepared<A, RA, RB, RC, RD> extends
      AbstractPreparedTransformer1<A, Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>> implements
      PreparedDAGTransformer<Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>> {
    ...
    * @param input1 the {@link Producer} providing the first input to the transformer
     */
    Prepared(DAGStructure<Tuple4<RA, RB, RC, RD>> dag, Producer<? extends A> input1) {
      super(input1);
      _dag = dag;
      _executor = new LocalDAGExecutor();
    }
     ...
     */
    @Override
    public DAG1x4.Prepared<A, RA, RB, RC, RD> withReduction(Reducer.Level level) {
      if (Reducer.Level.compare(level, _reductionLevel) >= 0) {
        return this;
      }
    ...
    public class InternalAPI extends
        AbstractPreparedTransformer1<A, Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>>.InternalAPI
        implements PreparedDAGTransformer.InternalAPI<Tuple4<RA, RB, RC, RD>, DAG1x4.Prepared<A, RA, RB, RC, RD>> {
      @Override
      public DAGStructure<Tuple4<RA, RB, RC, RD>> getDAGStructure() {
        return _dag;
      }
      ...
      @Override
      public DAG1x4.Prepared<A, RA, RB, RC, RD> getInstance() {
        return DAG1x4.Prepared.this;
      }
    }

    @Override
    protected Collection<? extends Reducer<? super DAG1x4.Prepared<A, RA, RB, RC, RD>>> getGraphReducers() {
      return Collections.singletonList(DAGTransformerReducer.INSTANCE);
    }
     ...
     * @return a copy of this instance that will use the provided executor
     */
    public DAG1x4.Prepared<A, RA, RB, RC, RD> withExecutor(PreparedDAGExecutor executor) {
      return clone(r -> r._executor = executor);
    }
     ...
     * @return a copy of this instance that will have the specified parent
     */
    public DAG1x4.Prepared<A, RA, RB, RC, RD> withInput(Producer<? extends A> input1) {
      return super.withAllInputs(input1);
    }
