...
/**
 * Pools a sequence of vectors from the input layer via mean pooling to create a single vector output (each element in
 * the output is the mean over all the corresponding elements in the input vectors).
 *
 * The size of the output vector is the same size as each of the vectors in the input sequence.
 */
@VisitedBy("NNLayerVisitor")
public class NNMeanPoolingLayer extends AbstractVectorSequencePoolingLayer<NNMeanPoolingLayer> {
   ...
   */
  public NNMeanPoolingLayer withDropoutProbability(double probability) {
