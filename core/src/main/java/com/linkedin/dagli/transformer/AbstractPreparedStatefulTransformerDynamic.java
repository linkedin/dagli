...
/**
 * Base class for prepared dynamic transformers that extends {@link AbstractPreparedTransformerDynamic} to
 ...
 *
 */
public abstract class AbstractPreparedStatefulTransformerDynamic<R, Q, S extends AbstractPreparedStatefulTransformerDynamic<R, Q, S>>
    extends AbstractPreparedTransformerDynamic<R, S> {
   ...
   */
  public AbstractPreparedStatefulTransformerDynamic(Producer<?>... inputs) {
    super(inputs);
  }
   ...
   */
  public AbstractPreparedStatefulTransformerDynamic(List<? extends Producer<?>> inputs) {
    super(inputs);
  }
  ...
  class InternalAPI extends AbstractPreparedTransformerDynamic<R, S>.InternalAPI {
    @Override
    @SuppressWarnings("unchecked")
    public R applyUnsafe(Object executionCache, Object[] values) {
      return AbstractPreparedStatefulTransformerDynamic.this.apply((Q) executionCache,
          new UnmodifiableArrayList<>(values, getArity()));
    }
    ...
    public void applyAllUnsafe(Object executionCache, int count, Object[][] values, Object[] results) {
      int arity = getArity();
      ArrayList<List<Object>> hyperlist = new ArrayList<>(arity);

      for (int i = 0; i < arity; i++) {
        hyperlist.add(new UnmodifiableArrayList<>(values[i], count));
      }

      AbstractPreparedStatefulTransformerDynamic.this.applyAll((Q) executionCache, hyperlist,
    ...
    public void applyAllUnsafe(Object executionCache, int count, List<? extends List<?>> values,
      ...
      AbstractPreparedStatefulTransformerDynamic.this.applyAll((Q) executionCache,
          Lists.limit(values, getArity(), count), results);
    }

    @Override
    public Q createExecutionCache(long exampleCountGuess) {
      return AbstractPreparedStatefulTransformerDynamic.this.createExecutionCache(exampleCountGuess);
    }

    @Override
    public int getPreferredMinibatchSize() {
      return AbstractPreparedStatefulTransformerDynamic.this.getPreferredMinibatchSize();
    }
  }
   ...
   *
   * The provided values list should neither be modified nor stored (after the method returns): its underlying data
   ...
   */
  protected abstract R apply(Q executionCache, List<?> values);
   ...
   *
   * Additionally, {@code values} (and its contained lists) must not be modified, and neither it nor the {@code results}
   * list should be stored past the end of the method call since the underlying data structures may be reused and
   * modified by Dagli after this call is finished.
   *
   ...
   * @param values a list of lists of input value lists corresponding to the examples being transformed; must not
   *               be modified or stored past the end of the method call
   * @param results an array to which should be added the results of applying this transformer to the given examples, in
   *                the same order the examples were provided; must not be stored past the end of the method call
   */
  protected void applyAll(Q executionCache, List<? extends List<?>> values, List<? super R> results) {
    assert values.size() == getInputList().size();

    int exampleCount = values.get(0).size();
    ArrayList<Object> exampleValueList = new ArrayList<>(values.size());
