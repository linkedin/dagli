...
/**
 * Base class for prepared transformers of arity 4 that extends {@link AbstractPreparedTransformer4} to
 ...
 */
public abstract class AbstractPreparedStatefulTransformer4<A, B, C, D, R, Q, S extends AbstractPreparedTransformer4<A, B, C, D, R, S>>
    extends AbstractPreparedTransformer4<A, B, C, D, R, S> {

  private static final long serialVersionUID = 1;

  public AbstractPreparedStatefulTransformer4() {
    super();
  }

  public AbstractPreparedStatefulTransformer4(Producer<? extends A> input1, Producer<? extends B> input2,
  ...
  public R apply(A value1, B value2, C value3, D value4) {
    return apply(createExecutionCache(1), value1, value2, value3, value4);
  }
   ...
   */
  protected abstract R apply(Q executionCache, A value1, B value2, C value3, D value4);
  ...
  protected void applyAll(Q executionCache, List<? extends A> values1, List<? extends B> values2,
    ...
    for (int i = 0; i < values1.size(); i++) {
      results.add(apply(executionCache, values1.get(i), values2.get(i), values3.get(i), values4.get(i)));
    }
  }
  ...
  protected class InternalAPI extends AbstractPreparedTransformer4<A, B, C, D, R, S>.InternalAPI {
    @Override
    public R apply(Object executionCache, A value1, B value2, C value3, D value4) {
      return AbstractPreparedStatefulTransformer4.this.apply((Q) executionCache, value1, value2, value3, value4);
    }

    @Override
    public Object createExecutionCache(long exampleCountGuess) {
      return AbstractPreparedStatefulTransformer4.this.createExecutionCache(exampleCountGuess);
    }
    ...
    public void applyAllUnsafe(Object executionCache, int count, Object[][] values, Object[] results) {
      AbstractPreparedStatefulTransformer4.this.applyAll((Q) executionCache, new UnmodifiableArrayList<>(
          (A[]) values[0], count), new UnmodifiableArrayList<>((B[]) values[1], count), new UnmodifiableArrayList<>(
          (C[]) values[2], count), new UnmodifiableArrayList<>((D[]) values[3], count), new FixedCapacityArrayList<>(
          results));
    }
    ...
    public void applyAllUnsafe(Object executionCache, int count, List<? extends List<?>> values, List<? super R> results) {
      AbstractPreparedStatefulTransformer4.this.applyAll((Q) executionCache,
          (List<A>) Lists.limit(values.get(0), count), (List<B>) Lists.limit(values.get(1), count),
          (List<C>) Lists.limit(values.get(2), count), (List<D>) Lists.limit(values.get(3), count), results);
    }

    @Override
    public int getPreferredMinibatchSize() {
      return AbstractPreparedStatefulTransformer4.this.getPreferredMinibatchSize();
    }
  }
}
