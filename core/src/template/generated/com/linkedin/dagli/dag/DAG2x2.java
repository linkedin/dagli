 ...
 *
 * DAG2x2 is a <i>preparable</i> DAG.
 *
 ...
 *
 * DAG2x2 is also a {@link com.linkedin.dagli.transformer.PreparableTransformer} and can be used as
 ...
 */
@ValueEquality
public class DAG2x2<A, B, RA, RB> extends
    AbstractPreparableTransformer2<A, B, Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>, DAG2x2<A, B, RA, RB>> implements
    PreparableDAGTransformer<Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>, DAG2x2<A, B, RA, RB>> {
   ...
   */
  DAG2x2(DAGStructure<Tuple2<RA, RB>> dag, Producer<? extends A> input1, Producer<? extends B> input2) {
    super(input1, input2);
    _dag = dag;
   ...
   */
  @Override
  public DAG2x2<A, B, RA, RB> withReduction(Reducer.Level level) {
  ...
  public class InternalAPI
      extends
      AbstractPreparableTransformer2<A, B, Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>, DAG2x2<A, B, RA, RB>>.InternalAPI
      implements
      PreparableDAGTransformer.InternalAPI<Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>, DAG2x2<A, B, RA, RB>> {
    @Override
    ...
    @Override
    public DAG2x2<A, B, RA, RB> getInstance() {
      return DAG2x2.this;
    }
  }

  @Override
  protected Collection<? extends Reducer<? super DAG2x2<A, B, RA, RB>>> getGraphReducers() {
   ...
   */
  public DAG2x2<A, B, RA, RB> withExecutor(DAGExecutor executor) {
   ...
   */
  public DAG2x2<A, B, RA, RB> withInputs(Producer<? extends A> input1, Producer<? extends B> input2) {
   ...
   */
  public DAG1x2<B, RA, RB> withGeneratorAsInput1(Generator<A> generator) {
    Placeholder<B> nestedPlaceholder2 = new Placeholder<>("Original Input 2");
    DAG2x2<A, B, RA, RB> dag = this.withInputs(generator, nestedPlaceholder2);

    return DAG.withPlaceholder(nestedPlaceholder2).withOutputs(new Value0FromTuple<>(dag), new Value1FromTuple<>(dag))
   ...
   *         the produced by the provided generator
   */
  public DAG1x2<A, RA, RB> withGeneratorAsInput2(Generator<B> generator) {
    Placeholder<A> nestedPlaceholder1 = new Placeholder<>("Original Input 1");
    DAG2x2<A, B, RA, RB> dag = this.withInputs(nestedPlaceholder1, generator);

    return DAG.withPlaceholder(nestedPlaceholder1).withOutputs(new Value0FromTuple<>(dag), new Value1FromTuple<>(dag))
   ...
   * @param <RB> the type of the second result
   */
  private static class Preparer<A, B, RA, RB> extends
      AbstractBatchPreparer2<A, B, Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>> {
    // the DAG being prepared
    private final DAG2x2<A, B, RA, RB> _dag;
    ...
    public PreparerResultMixed<PreparedTransformer2<A, B, Tuple2<RA, RB>>, DAG2x2.Prepared<A, B, RA, RB>> finish(
  ...
  @Override
  protected Preparer2<A, B, Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>> getPreparer(PreparerContext context) {
   ...
   *         preparation data
   */
  public Result<A, B, RA, RB> prepareAndApply(Iterable<? extends A> values1, Iterable<? extends B> values2) {
    DAGExecutionResult<Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>> res =
        _executor.internalAPI().prepareAndApplyUnsafe(this,
            new ObjectReader[] { ObjectReader.wrap(values1), ObjectReader.wrap(values2) });
    return new Result<>((DAG2x2.Prepared<A, B, RA, RB>) res.getPreparerResult().getPreparedTransformerForNewData(),
        res.getOutputs());
  }
   ...
   */
  public DAG2x2.Prepared<A, B, RA, RB> prepare(Iterable<? extends A> values1, Iterable<? extends B> values2) {
    return (DAG2x2.Prepared<A, B, RA, RB>) _executor.internalAPI()
  ...
  * @param <RB> the type of the second result
   */
  public static final class Result<A, B, RA, RB> extends AbstractDAGResult2<RA, RB> {
    // the prepared DAG
    private final DAG2x2.Prepared<A, B, RA, RB> _preparedDAG;

    /**
     * @return the {@link Prepared} DAG that was prepared (trained) on the provided preparation (training) data.
     */
    public DAG2x2.Prepared<A, B, RA, RB> getPreparedDAG() {
      return _preparedDAG;
    }
     ...
     */
    Result(DAG2x2.Prepared<A, B, RA, RB> preparedDAG, ObjectReader<?>[] results) {
      super(results);
  ...
  * @param <RB> the type of the second result
   */
  @ValueEquality
  public static class Prepared<A, B, RA, RB> extends
      AbstractPreparedTransformer2<A, B, Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>> implements
      PreparedDAGTransformer<Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>> {
    ...
    * @param input2 the {@link Producer} providing the second input to the transformer
     ...
     */
    @Override
    public DAG2x2.Prepared<A, B, RA, RB> withReduction(Reducer.Level level) {
      if (Reducer.Level.compare(level, _reductionLevel) >= 0) {
        return this;
      }
    ...
    public class InternalAPI extends
        AbstractPreparedTransformer2<A, B, Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>>.InternalAPI implements
        PreparedDAGTransformer.InternalAPI<Tuple2<RA, RB>, DAG2x2.Prepared<A, B, RA, RB>> {
      @Override
      public DAGStructure<Tuple2<RA, RB>> getDAGStructure() {
        return _dag;
      }
      ...
      @Override
      public DAG2x2.Prepared<A, B, RA, RB> getInstance() {
        return DAG2x2.Prepared.this;
      }
    }

    @Override
    protected Collection<? extends Reducer<? super DAG2x2.Prepared<A, B, RA, RB>>> getGraphReducers() {
      return Collections.singletonList(DAGTransformerReducer.INSTANCE);
    }
     ...
     * @return a copy of this instance that will use the provided executor
     */
    public DAG2x2.Prepared<A, B, RA, RB> withExecutor(PreparedDAGExecutor executor) {
      return clone(r -> r._executor = executor);
    }
     ...
     * @return a copy of this instance that will have the specified parents
     */
    public DAG2x2.Prepared<A, B, RA, RB> withInputs(Producer<? extends A> input1, Producer<? extends B> input2) {
      return super.withAllInputs(input1, input2);
    }
     ...
     *         the produced by the provided generator
     */
    public DAG1x2.Prepared<B, RA, RB> withGeneratorAsInput1(Generator<A> generator) {
      Placeholder<B> nestedPlaceholder2 = new Placeholder<>("Original Input 2");
      DAG2x2.Prepared<A, B, RA, RB> dag = this.withInputs(generator, nestedPlaceholder2);
     ...
     *         the produced by the provided generator
     */
    public DAG1x2.Prepared<A, RA, RB> withGeneratorAsInput2(Generator<B> generator) {
      Placeholder<A> nestedPlaceholder1 = new Placeholder<>("Original Input 1");
      DAG2x2.Prepared<A, B, RA, RB> dag = this.withInputs(nestedPlaceholder1, generator);
    ...
    @Override
    public Tuple2<RA, RB> apply(A value1, B value2) {
      try (
