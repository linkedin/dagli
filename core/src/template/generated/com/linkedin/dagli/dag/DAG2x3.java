 ...
 *
 * DAG2x3 is a <i>preparable</i> DAG.
 *
 ...
 *
 * DAG2x3 is also a {@link com.linkedin.dagli.transformer.PreparableTransformer} and can be used as
 ...
 */
@ValueEquality
public class DAG2x3<A, B, RA, RB, RC>
    extends
    AbstractPreparableTransformer2<A, B, Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>, DAG2x3<A, B, RA, RB, RC>>
    implements
    PreparableDAGTransformer<Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>, DAG2x3<A, B, RA, RB, RC>> {
   ...
   */
  DAG2x3(DAGStructure<Tuple3<RA, RB, RC>> dag, Producer<? extends A> input1, Producer<? extends B> input2) {
    super(input1, input2);
    _dag = dag;
   ...
   */
  @Override
  public DAG2x3<A, B, RA, RB, RC> withReduction(Reducer.Level level) {
  ...
  public class InternalAPI
      extends
      AbstractPreparableTransformer2<A, B, Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>, DAG2x3<A, B, RA, RB, RC>>.InternalAPI
      implements
      PreparableDAGTransformer.InternalAPI<Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>, DAG2x3<A, B, RA, RB, RC>> {
    @Override
    ...
    @Override
    public DAG2x3<A, B, RA, RB, RC> getInstance() {
      return DAG2x3.this;
    }
  }

  @Override
  protected Collection<? extends Reducer<? super DAG2x3<A, B, RA, RB, RC>>> getGraphReducers() {
   ...
   */
  public DAG2x3<A, B, RA, RB, RC> withExecutor(DAGExecutor executor) {
   ...
   */
  public DAG2x3<A, B, RA, RB, RC> withInputs(Producer<? extends A> input1, Producer<? extends B> input2) {
   ...
   */
  public DAG1x3<B, RA, RB, RC> withGeneratorAsInput1(Generator<A> generator) {
    Placeholder<B> nestedPlaceholder2 = new Placeholder<>("Original Input 2");
    DAG2x3<A, B, RA, RB, RC> dag = this.withInputs(generator, nestedPlaceholder2);

    return DAG.withPlaceholder(nestedPlaceholder2)
   ...
   *         the produced by the provided generator
   */
  public DAG1x3<A, RA, RB, RC> withGeneratorAsInput2(Generator<B> generator) {
    Placeholder<A> nestedPlaceholder1 = new Placeholder<>("Original Input 1");
    DAG2x3<A, B, RA, RB, RC> dag = this.withInputs(nestedPlaceholder1, generator);

    return DAG.withPlaceholder(nestedPlaceholder1)
        .withOutputs(new Value0FromTuple<>(dag), new Value1FromTuple<>(dag), new Value2FromTuple<>(dag))
   ...
   * @param <RC> the type of the third result
   */
  private static class Preparer<A, B, RA, RB, RC> extends
      AbstractBatchPreparer2<A, B, Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>> {
    // the DAG being prepared
    private final DAG2x3<A, B, RA, RB, RC> _dag;
     ...
     */
    Preparer(DAG2x3<A, B, RA, RB, RC> dag) {
      _dag = dag;
    }
    ...
    public PreparerResultMixed<PreparedTransformer2<A, B, Tuple3<RA, RB, RC>>, DAG2x3.Prepared<A, B, RA, RB, RC>> finish(
  ...
  @Override
  protected Preparer2<A, B, Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>> getPreparer(PreparerContext context) {
   ...
   *         preparation data
   */
  public Result<A, B, RA, RB, RC> prepareAndApply(Iterable<? extends A> values1, Iterable<? extends B> values2) {
    DAGExecutionResult<Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>> res =
        _executor.internalAPI().prepareAndApplyUnsafe(this,
            new ObjectReader[] { ObjectReader.wrap(values1), ObjectReader.wrap(values2) });
    return new Result<>((DAG2x3.Prepared<A, B, RA, RB, RC>) res.getPreparerResult().getPreparedTransformerForNewData(),
        res.getOutputs());
  }
   ...
   */
  public DAG2x3.Prepared<A, B, RA, RB, RC> prepare(Iterable<? extends A> values1, Iterable<? extends B> values2) {
    return (DAG2x3.Prepared<A, B, RA, RB, RC>) _executor.internalAPI()
  ...
  * @param <RC> the type of the third result
   */
  public static final class Result<A, B, RA, RB, RC> extends AbstractDAGResult3<RA, RB, RC> {
    // the prepared DAG
    private final DAG2x3.Prepared<A, B, RA, RB, RC> _preparedDAG;

    /**
     * @return the {@link Prepared} DAG that was prepared (trained) on the provided preparation (training) data.
     */
    public DAG2x3.Prepared<A, B, RA, RB, RC> getPreparedDAG() {
      return _preparedDAG;
    }
     ...
     */
    Result(DAG2x3.Prepared<A, B, RA, RB, RC> preparedDAG, ObjectReader<?>[] results) {
      super(results);
  ...
  * @param <RC> the type of the third result
   */
  @ValueEquality
  public static class Prepared<A, B, RA, RB, RC> extends
      AbstractPreparedTransformer2<A, B, Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>> implements
      PreparedDAGTransformer<Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>> {
    ...
    * @param input2 the {@link Producer} providing the second input to the transformer
     ...
     */
    @Override
    public DAG2x3.Prepared<A, B, RA, RB, RC> withReduction(Reducer.Level level) {
      if (Reducer.Level.compare(level, _reductionLevel) >= 0) {
        return this;
      }
    ...
    public class InternalAPI extends
        AbstractPreparedTransformer2<A, B, Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>>.InternalAPI implements
        PreparedDAGTransformer.InternalAPI<Tuple3<RA, RB, RC>, DAG2x3.Prepared<A, B, RA, RB, RC>> {
      @Override
      public DAGStructure<Tuple3<RA, RB, RC>> getDAGStructure() {
        return _dag;
      }
      ...
      @Override
      public DAG2x3.Prepared<A, B, RA, RB, RC> getInstance() {
        return DAG2x3.Prepared.this;
      }
    }

    @Override
    protected Collection<? extends Reducer<? super DAG2x3.Prepared<A, B, RA, RB, RC>>> getGraphReducers() {
      return Collections.singletonList(DAGTransformerReducer.INSTANCE);
    }
     ...
     * @return a copy of this instance that will use the provided executor
     */
    public DAG2x3.Prepared<A, B, RA, RB, RC> withExecutor(PreparedDAGExecutor executor) {
      return clone(r -> r._executor = executor);
    }
     ...
     * @return a copy of this instance that will have the specified parents
     */
    public DAG2x3.Prepared<A, B, RA, RB, RC> withInputs(Producer<? extends A> input1, Producer<? extends B> input2) {
      return super.withAllInputs(input1, input2);
    }
     ...
     *         the produced by the provided generator
     */
    public DAG1x3.Prepared<B, RA, RB, RC> withGeneratorAsInput1(Generator<A> generator) {
      Placeholder<B> nestedPlaceholder2 = new Placeholder<>("Original Input 2");
      DAG2x3.Prepared<A, B, RA, RB, RC> dag = this.withInputs(generator, nestedPlaceholder2);

      return DAG.Prepared.withPlaceholder(nestedPlaceholder2)
          .withOutputs(new Value0FromTuple<>(dag), new Value1FromTuple<>(dag), new Value2FromTuple<>(dag))
          .withExecutor(_executor).withInput(getInput2());
    }
     ...
     *         the produced by the provided generator
     */
    public DAG1x3.Prepared<A, RA, RB, RC> withGeneratorAsInput2(Generator<B> generator) {
      Placeholder<A> nestedPlaceholder1 = new Placeholder<>("Original Input 1");
      DAG2x3.Prepared<A, B, RA, RB, RC> dag = this.withInputs(nestedPlaceholder1, generator);

      return DAG.Prepared.withPlaceholder(nestedPlaceholder1)
          .withOutputs(new Value0FromTuple<>(dag), new Value1FromTuple<>(dag), new Value2FromTuple<>(dag))
          .withExecutor(_executor).withInput(getInput1());
    }
    ...
    @Override
    public Tuple3<RA, RB, RC> apply(A value1, B value2) {
      try (
